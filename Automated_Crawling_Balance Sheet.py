...
def get_資產負債表(TYPEK, year, season, retries=3):
...
def 自動抓取資料(TYPEK, start_year, start_season, **kwargs):
    ...
    while (year < current_year) or (year == current_year and season <= current_season):
        ...
        try:
            df = get_資產負債表(TYPEK, year, season)
    ...
    try:
        all_data.to_sql('projectdata2', con=engine, if_exists='replace', index=False)
        print("資料匯入成功！")
    except Exception as e:
        print(f"發生錯誤：{e}")
    ...
    with engine.connect() as connection:
        result = connection.execute(text("SELECT COUNT(*) FROM projectdata2"))
        count = result.scalar()
        print(f"目前在 projectdata2 表中的資料量為: {count} 筆")
